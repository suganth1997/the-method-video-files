...
class ParabolaTrianglesPlot(MovingCameraScene):
    def construct(self):
        ...
        parabola_graph = axes.get_graph(lambda x: f(x), color=YELLOW, x_range=[x1_, x2_, 0.01])

        triangle_lines = [[{'m':m_base, 'c':c_base, 'x1':x1_, 'x2':x2_}]]

        triangle_lines_obj = [[axes.get_graph(lambda x: triangle_lines[0][0]['m']*x + triangle_lines[0][0]['c'], color=YELLOW, x_range=[x1_, x2_, 0.01])]]

        for i_div in range(divs):
            triangle_lines_now = []
            triangle_lines_now_obj = []
            for bases in triangle_lines[i_div]:
                x1 = bases['x1']
                y1 = f(x1)
                ...
                triangle_lines_now.append({'m':m, 'c':c, 'x1':x1, 'x2':x3})
                triangle_lines_now_obj.append(axes.get_graph(lambda x: m*x+c, color=YELLOW, x_range=[x1, x3, 0.01], stroke_width=DEFAULT_STROKE_WIDTH/(i_div + 1)))
                ...
                triangle_lines_now.append({'m':m, 'c':c, 'x1':x2, 'x2':x3})
                triangle_lines_now_obj.append(axes.get_graph(lambda x: m*x+c, color=YELLOW, x_range=[x2, x3, 0.01], stroke_width=DEFAULT_STROKE_WIDTH/(i_div + 1)))
