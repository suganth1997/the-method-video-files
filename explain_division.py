...
class ExplainDivision(MovingCameraScene):
    def construct(self):
        ...
        self.play(Uncreate(line_1), Uncreate(line_2), Uncreate(numberLine_prev_group), Uncreate(N_acc_1_2))
        ...
        fx00 = MathTex("f(x) = e^{-x} - e^{-1.617}").move_to([4, 1.75, 0])
        ...
        self.play(
            ApplyMethod(fx0.become, fx00), 
            # ApplyMethod(n_steps_str.become, Tex("N = ", str(0)).align_on_border(LEFT).shift(DOWN/2)),
            # ApplyMethod(b_a_eq.become, MathTex("b-a=").next_to(n_steps_str).shift(RIGHT)),
            ApplyMethod(accuracy_line.become, VGroup(
                Tex("N = ", str(0)).align_on_border(LEFT).shift(DOWN/2),
                MathTex("b-a=").next_to(n_steps_str).shift(RIGHT),
                DecimalNumber(1.0, num_decimal_places=4).next_to(b_a_eq)
            )),
            ApplyMethod(numberLine_prev_group.become, VGroup(text_a, text_b, numberLine_prev))
        )

        f = lambda x : exp(-x) - exp(-1.617)
        ...
        for i in range(12):
            ...
            if i == 3:
                ...
                interval_len = VGroup(log_simeq, DecimalNumber(log2(10)).next_to(log_simeq))
                self.play(Create(interval_len))
            ...
            if i == 9:
                self.play(ApplyMethod(accuracy_line.shift, 0.5*LEFT))
                surround_accuracy = SurroundingRectangle(accuracy_value)
                self.play(Create(surround_accuracy))
                less_than_0_1 = MathTex("\leq 0.001").next_to(surround_accuracy)
                self.play(Write(less_than_0_1))
        ...
        self.play(ApplyMethod(sub_group.become, Tex("It depends on the bracket we choose").next_to(question_1, DOWN).to_edge(LEFT)))
